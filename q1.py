...
class q1():
    ...
    def resolveProblem(self):
        ...
        for i in range(num_offer):
            self.solver.Add(self.solver.Sum([x[i, j] for j in range(num_demand)]) <= self.offers[i])

        # Restrições de demanda
        for j in range(num_demand):
            self.solver.Add(self.solver.Sum([x[i, j] for i in range(num_offer)]) >= self.demands[j])
        ...
        if status == pywraplp.Solver.OPTIMAL or status == pywraplp.Solver.FEASIBLE:
            ...
            textfile.write('\n Array de demandas : {} \n Soma das demandas : {} \n'.format(self.demands, sum(self.demands)))
            textfile.write('\n Array de ofertas : {} \n Soma das Ofertas: {} \n'.format(self.offers, sum(self.offers)))
            textfile.write('\n Matriz de custos : {} \n'.format(self.costs))
            textfile.close()